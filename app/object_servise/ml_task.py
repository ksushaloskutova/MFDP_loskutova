...
class MLTaskAdd(SQLModel, table=True):
    ...
    password: str = Field(
        default_factory=lambda: generate_password(6)
    )  # ⚠️ lambda для аргуме
    ...
    def prepare_image(self, input_data: Image.Image) -> np.ndarray:
        ...
        try:
            ...
            if image_array.shape != (1, 224, 224, 3):
                raise ValueError(
                    f"Некорректная размерность после обработки: {image_array.shape}"
                )
    ...
    def predict(self, prepared_array: np.ndarray, model: MLModel) -> np.ndarray:
        ...
        try:
            ...
            if prepared_array.shape != (1, 224, 224, 3):
                raise ValueError(
                    f"Ожидается (1,224,224,3), получено: {prepared_array.shape}"
                )
    ...
    def save_images(
        self, input_data: Image.Image, directory_path: str = "images"
    ) -> None:
        try:
            # Создаем директорию, если её нет
            os.makedirs(directory_path, exist_ok=True)

            logger.info("Сохраняем изображение в формате: jpg")
            ...
            )  # Используем верхний регистр для формата JPG

            # Добавляем в DVC
            result = subprocess.run(
                ["dvc", "add", directory_path],
                check=False,
                capture_output=True,
                text=True,
            )
            ...
            # Пушим изменения
            result = subprocess.run(
                ["dvc", "push"], check=False, capture_output=True, text=True
            )
            if result.returncode != 0:
