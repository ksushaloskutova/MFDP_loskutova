...
async def retrieve_task_result(
    ...
    if not (MLTaskServise.password_verify(task, password)):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="the password is incorrect"
        )
    ...
    if not image:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="I can't upload the image"
        )
...
async def create_task(file: UploadFile = File(...), session=Depends(get_session)):
    ...
    try:
        ...
        return {
            "message": "Your photo has been sent for processing.",
