...
from interaction_servise import token_interaction, user_interaction
from object_servise.user import UserResponse
...
async def login_post(data: UserResponse, session=Depends(get_session)):
    ...
    token = token_interaction.create_access_token(
        {"chat_id": chat_id, "login": login}, session
    )
...
async def register_post(data: UserResponse, session=Depends(get_session)):
    ...
    try:
        create_check = user_interaction.create_user(
            UserResponse(login=login, password=password), session
        )
...
async def logout_delete_token(
    ...
    try:
        ...
        if check_delete:
            return {"status": "success_logout"}
        else:
            return {"status": "success_not_login"}
