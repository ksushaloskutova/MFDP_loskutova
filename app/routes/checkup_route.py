...
from object_servise.checkup import CheckupResponse
...
async def create_checkup(data: CheckupResponse, session=Depends(get_session)):
    if not checkup_interaction.check_time_available(
        data.checkup_time, data.place, session
    ):
        raise HTTPException(
    ...
    if not checkup_interaction.check_login_available(
        data.checkup_time, data.login, session
    ):
        last_checkup = checkup_interaction.get_checkup_last_by_login(
            data.login, session
        )
        last_checkup = last_checkup["result"]
        last_date = (
            last_checkup.checkup_time.strftime("%d.%m.%Y") if last_checkup else ""
        )
        raise HTTPException(
...
async def available_time(
    ...
    available_time = checkup_interaction.get_available_time_slots(
        session, date, hour_range, place
    )
