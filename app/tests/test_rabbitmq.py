...
def test_consumer_with_dummy_model(monkeypatch, rabbitmq_connection):
    ...
    monkeypatch.setattr(
        ch,
        'basic_consume',
        lambda queue, on_message_callback: on_message_callback(
            ch,
            type('M', (), {'delivery_tag': 1}),
            type('P', (), {}),
            b'{"task_id":"x","password":"y","image":""}',
        ),
    )
    # Мокаем acknowledge
    monkeypatch.setattr(
        ch, 'basic_ack', lambda delivery_tag: setattr(dummy, 'called', True)
    )
    consume_tasks(dummy)
