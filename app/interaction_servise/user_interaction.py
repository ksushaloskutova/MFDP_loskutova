...
def create_admin(session) -> None:
    admin = User(user=UserResponse(login=get_settings().ADMIN_LOGIN,
                                           password=get_settings().ADMIN_PASSWORD), role="admin")
...
def create_test_user(session) -> None:
    test_user = User(user=UserResponse(login="testov@mail.ru", password="12345a"), role="user")
    with session:
