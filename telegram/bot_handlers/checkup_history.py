...
def check_my_checkup_records(bot_instance):
    @bot_instance.callback_query_handler(
        func=lambda call: call.data == 'my_checkup_now'
    )
    @auth_required
        ...
        try:
            ...
            if hasattr(message, "data"):  # Проверяем, что это callback
                bot_instance.answer_callback_query(
                    message.id, "⌛ Запрос обрабатывается..."
                )

            # Быстрый запрос к API (timeout=5 сек)
            response = requests.get(
                f"{API_URL}/checkup/last", params={"login": login}, timeout=5
            )
            ...
            if hasattr(message, "message"):  # Если это callback, редактируем сообщение
                bot_instance.edit_message_text(
                    ...
                    text=text,
                    reply_markup=(
                        get_booking_keyboard() if response.status_code == 404 else None
                    ),
                )
